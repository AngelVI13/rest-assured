...
public class PostResults {
    ...
    private void post(int testRailId, Map data) {
        try {
            JSONObject r = (JSONObject) getTestRailAPIClient().sendPost("add_result/" + testRailId + "/DemoTestRun", data);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private APIClient getTestRailAPIClient() {
        APIClient client = new APIClient("https://nymbusdemo.testrail.io/");
        client.setUser("angel.iliev@auriga.com");
